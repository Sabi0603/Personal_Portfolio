        ...
        {!loading && !error && certifications.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {certifications.map((cert) => {
              const isPdf = isPdfCertificate(cert);
              const targetUrl = isPdf
                ? getCertificationViewUrl(cert._id)
                : cert.image?.url;
              const previewSrc = isPdf
                ? cert.image?.previewUrl || getCertificationPreviewUrl(cert._id)
                : cert.image?.url;
              ...
              return (
                ...
                >
                  <div className="space-y-4">
                    ...
                    {/* Certificate Media Preview (Image or PDF first-page preview) */}
                    {previewSrc && (
                      <a
                        href={targetUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="relative w-full h-44 sm:h-48 md:h-52 rounded-xl overflow-hidden bg-slate-950/40 dark:bg-slate-900/60 border border-(--border-color)/80 flex items-center justify-center p-3 group/preview hover:border-cyan-500/40 transition-colors cursor-pointer block"
                        title={isPdf ? 'View PDF Certificate' : 'View Certificate Image'}
                      >
                        <img
                          src={previewSrc}
                          alt={`${cert.title} certificate`}
                          className="w-full h-full object-contain rounded-lg transition-transform duration-300 group-hover/preview:scale-[1.02]"
                          loading="lazy"
                          onError={(e) => {
                            if (isPdf && e.target.src !== getCertificationPreviewUrl(cert._id)) {
                  ...
                  {/* Verification & View Links */}
                  <div className="pt-2 flex flex-wrap items-center justify-between gap-2 border-t border-(--border-color)/40">
                    {cert.credentialUrl ? (
                      <a
                    ...
                    {targetUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 text-xs font-mono text-(--text-muted) hover:text-cyan-400 transition-colors ml-auto"
                      >
